...
3. Use `.ai/routing.md` to pick which pack(s) the current task type needs — don't load packs unrelated to the task.
4. Open a full `docs/*.md` file only if its pack's summary doesn't resolve the question.
5. Do not open `keystatic.config.ts` and `src/content.config.ts` unless the task actually touches content schema — most tasks don't need either.
6. Do not re-read `package.json`, `astro.config.mjs`, or design tokens speculatively — they rarely change and their relevant facts are already captured in `okf/audit/current-state.okf.md`.
