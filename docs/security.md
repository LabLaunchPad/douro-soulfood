...
- **HTTP security headers** (`public/_headers`): `X-Frame-Options: DENY`, `X-Content-Type-Options: nosniff`, `Referrer-Policy: strict-origin-when-cross-origin`, `Permissions-Policy: camera=(), microphone=(), geolocation=()`, `Strict-Transport-Security` with `includeSubDomains`, and a CSP: `default-src 'self'; script-src 'self' 'sha256-...' (x5, one per inline script — see §"Known, verified gap" below); style-src 'self' 'unsafe-inline'; font-src 'self'; img-src 'self' data: https://images.unsplash.com; frame-src https://maps.google.com; frame-ancestors 'none'; object-src 'none'; base-uri 'self'; form-action 'self'`.
...
**Fix**: each inline script's exact SHA-256 content hash was added to `public/_headers`' `script-src` (5 hashes total). The JSON-LD block happens to be byte-identical across every page today (no page overrides `Base.astro`'s `image` prop), so one hash covers it everywhere; the same is true for the shared NavBar/mobile-menu scripts. Verified with a real headless-Chrome test: clicking the hamburger button now sets `#mobile-menu`'s `data-open` to `true`, clicking "Karte anzeigen" now loads the iframe, and zero CSP violations appear in the console.

**Maintenance requirement, not optional**: these hashes are pinned to exact minified byte output, which can shift on any Astro/Vite/esbuild version bump — not only on a hand-edited source change. Run `node scripts/checks/verify-csp-hashes.mjs` (after `pnpm build`) whenever `Base.astro`, `NavBar.astro`, `MobileNavDrawer.astro`, `MapEmbed.astro` change, or after any dependency update — it fails loudly if a hash is missing, rather than the previous failure mode (silent breakage, console-only, easy to miss without opening devtools).
