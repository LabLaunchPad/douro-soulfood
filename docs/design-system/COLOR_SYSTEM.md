...
There is no `--color-warning` token. Warning-adjacent UI (allergen badges, prep-time indicators) currently uses Tailwind's default `amber-*` palette directly (`AllergenBadge.astro`, `MenuItemCard.astro`, `MenuBistroCard.astro`) — 44 call sites total across `stone-*` (muted text) and `amber-*` (warning-ish accents), verified via grep 2026-08-07. **Recommendation**: add `--color-warning` and `--color-warning-surface` tokens (an amber/gold-family OKLCH pair, contrast-checked against `--color-surface-card`) and route these call sites through them — not urgent (they're internally consistent with each other today), but the right fix before the palette grows further.
