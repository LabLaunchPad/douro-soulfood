...
| Spacing | Tailwind's default 4px scale + 2 custom section tokens | [`SPACING_SYSTEM.md`](./SPACING_SYSTEM.md) | Good (75 documented half-step exceptions in compact UI, not silent). `--spacing-section-mobile` (72px) is now adopted as `py-section-mobile` at all 10 homepage section-wrapper declarations (6 in `index.astro`, 1 each in `PhotoGrid.astro`/`OurStorySection.astro`/`FaqAccordion.astro`/`UserReviews.astro` — `PhotoGrid.astro` renders twice on the homepage, so 11 visible section instances from 10 source declarations), with the `md:` desktop pairing standardized to `md:py-5` (20px) everywhere as of 2026-08-07's spacing-strict follow-up pass (was drifting between `md:py-3`/`md:py-4`/`md:py-5` across otherwise-identical sections). `--spacing-section` (desktop, 120px) remains unused — that's a separate, larger token still awaiting adoption, out of scope for a spacing-consistency pass |
