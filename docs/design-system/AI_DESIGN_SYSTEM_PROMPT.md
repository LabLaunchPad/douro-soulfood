...
**Current real status against this bar** (2026-08-07 audit, updated same day after fixes): colour is now fully token-governed (38/44 previously-un-tokenized call sites fixed; the remaining 6 verified as correctly context-specific amber accents, not a gap — see [`COLOR_SYSTEM.md`](./COLOR_SYSTEM.md)). Typography's token scale was realigned to match the real, disciplined 17-combination pattern already in use across 298 call sites — a token-definition fix, not a markup migration, verified zero visual change (see [`TYPOGRAPHY.md`](./TYPOGRAPHY.md)). Spacing's 75 half-step exceptions remain intentionally undone — investigated and judged to be defensible, context-appropriate choices for compact UI, not drift (see [`SPACING_SYSTEM.md`](./SPACING_SYSTEM.md)); forcing them onto a strict 4px grid would very likely make the affected badges/icon-gaps look worse, not better, which fails this system's own Clarity/Usability-over-novelty tradeoff rule.
