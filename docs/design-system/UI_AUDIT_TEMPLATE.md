...
## Worked example: 2026-08-07 mobile-first conversion redesign (deliberate visual/structural change, not a consistency fix)

**Method used**: this pass was different in kind from the ones above it — a real design brief (typography scale, spacing scale, card radius, button-height floor, section reordering) rather than a defect audit. Three genuine conflicts with the prior pass's own conclusions were surfaced and resolved via explicit user decisions before any edit: `--spacing-section-mobile` dropping from 72px to 32px (sitewide, not homepage-only), the tap-target floor rising from 44px to 48px (sitewide), and the scope extending to full section reordering, not just in-place restyling. One prior decision was explicitly reversed with the user's knowledge: `HeroSection`'s mobile CTA, deliberately removed in an earlier pass, was reinstated because this brief's "one dominant primary action" priority superseded the earlier "MobileBottomBar alone is sufficient" call.

**A real misconception caught during execution**: the plan initially assumed `rounded-2xl` was ~24px, matching the brief's 20–24px card-radius target. Reading `tokens.css` directly showed `--radius-2xl` is actually 40px (`--radius-xl` is 28px) — both over-spec. Only `--radius-lg` (20px) fits. Every homepage card using an unconditional (non-responsive) `rounded-xl`/`rounded-2xl` was corrected to `rounded-lg`; chip/tile instances that only switch to the larger radius at `sm:`+ were left alone since the brief's four target widths (360/390/414/430px) never reach that breakpoint.

**Real structure changes**: reordered the homepage from Hero→Reviews→Categories→PopularDishes→Story→Firmenevents→Testimonials→PrivateFeiern→Galerie→FAQ→Location to Hero→Reviews→**Quick-access (new)**→PopularDishes→Story→Testimonials→Categories→**Events (Firmenevents+PrivateFeiern consolidated into one two-card section)**→Galerie→FAQ→Location, per explicit conversion-flow reasoning agreed with the user. The consolidated Events section replaced two full-width `FeatureCard.astro` usages with inline two-up card markup (`FeatureCard.astro` itself is unaffected and still used by `catering.astro`/`dev/ui.astro`— verified via grep before assuming it was safe to stop using on the homepage).

**Fixed this pass**: `--spacing-section-mobile` → `2rem`; `min-h-11` → `min-h-12` across `Button.astro` (all three sizes, not just the two that previously used `min-h-11`) and six other components; `Button.astro` text bumped to `text-base` at every size; hero eyebrow → `text-xs`, section titles → `text-2xl`, category-grid labels → `text-xs`, several body-copy paragraphs → `text-base`; new quick-access chip row (Menu/Reservieren/Events/Standort, linking to a new `id="standort"` anchor); mobile hero CTA reinstated (`Speisekarte ansehen`, deliberately distinct from `MobileBottomBar`'s Call/Order to avoid redundancy); 5 card instances corrected from `rounded-xl`/`rounded-2xl` to `rounded-lg`, with `OurStorySection`'s and the relocated Map article's `shadow-lg` also stepped down to `shadow-md` per the brief's "subtle shadows only" rule.

**Verified**: `pnpm build` clean, CSP hash check passing, zero overflow/zero axe violations across the homepage at all 6 checkpoint widths (360/390/414/430/768/1440) plus a 6-route × 2-viewport regression sweep (the other 6 pages, since the spacing/button-height tokens changed sitewide); every CTA's rendered height measured via `boundingBox()` (not inferred from class names) confirming 48px; real full-page and component-zoom screenshots reviewed at mobile and desktop before considering any layout change (e.g. the consolidated Events cards) acceptable — two apparent defects (blank images in the Events section, a "null" hero-CTA measurement) were both confirmed as test-script/screenshot-timing artifacts via direct `naturalWidth`/`isVisible()` checks, not real bugs, before being dismissed.

**Deferred**: none new — this pass's scope was fully executed per the agreed plan; the `--spacing-section` (desktop) token remains a separate, larger, not-yet-scoped adoption effort.

---

## Worked example: 2026-08-08 multi-domain homepage audit (viewport × section × component × color × font × spacing)

**Method used**: real browser rendering only — full-page Playwright/Chromium screenshots at 360/390/430/768/1440px, plus `getComputedStyle()` extraction (not source-code inference) for section padding, heading typography, body-copy typography, CTA colors, and card radius across every homepage section. Cross-referenced against `tokens.css` rather than assumed from class names.

**Findings**: One real, measurable defect — the testimonials section had ~50% more surrounding whitespace (96px combined gap) than every other section boundary (64px), confirmed via `boundingBox()` measurement, not eyeballing. Root cause: `index.astro` wrapped `<UserReviews />` in its own `<section class="py-section-mobile md:py-5">`, while `UserReviews.astro`'s own root is already a full section carrying the same class — a pre-existing double-wrap (originally introduced alongside a *different*, already-fixed duplicate-`aria-label` issue in an earlier pass) that only became visually significant once the section-padding token grew to 32px. Everything else audited came back clean and consistent: all 9 section paddings measured exactly 32px, all section-title headings measured 24px/Fraunces/600-weight/identical color, all body-copy paragraphs measured 16px, all primary-CTA colors matched `--color-brand-gold`/`--color-brand-espresso` exactly, and card/chip radii measured the corrected 20px (`--radius-lg`) from the prior pass's fix.

**Fixed this pass**: removed the redundant outer `<section>` wrapper around `<UserReviews />` in `index.astro`, rendering it directly — matching how `PhotoGrid`/`OurStorySection`/`FaqAccordion` are already invoked without an extra wrapper. Updated every doc found making a now-stale claim about the old 72px mobile section rhythm or `MobileBottomBar`'s "always visible" state (both `docs/design-system/` and the parallel `docs/ui/` tree): `SPACING_SYSTEM.md`, `DESIGN_TOKENS.md`, `DESIGN_PRINCIPLES.md`, `design-system.md`, `docs/ui/RESPONSIVE_GUIDELINES.md`, `docs/ui/DESIGN_TOKENS.md`.

**Deferred**: none — this was a small, well-scoped fix with no open questions.
